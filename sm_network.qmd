...
seoul_sn <- st_cast(seoul_sn, "LINESTRING") # 반드시 수행해야 함.
```

서울시 도로망을 sfn 객체로 만든다.

```{r}
seoul_sfn <- as_sfnetwork(seoul_sn, directed = FALSE)
```
...
```{r}
dist_matrix_eucl <- st_distance(seoul_sobang)
```

#### 네트워크 거리 매트릭스

```{r}
dist_matrix_network <- st_network_cost(seoul_sfn, from = seoul_sobang, to = seoul_sobang, direction = "all")
```

## 최단 네트워크 경로

모든 소방서에서 종로소방서로 가는 최단 네트워크 경로를 구한다.

```{r}
jongro_st <- subset(seoul_sobang, Name == "종로소방서")
sobang_network_paths_sfn <- st_network_paths(seoul_sfn, from = jongro_st, to = seoul_sobang)
```

경로를 라인으로 표현하기 위해서는 paths_sobang_network 객체를 해부해야 한다. 함수를 작성한다.
...
```{r}
sobang_network_paths_sfn_lines <- snu_paths_generator(seoul_sn, sobang_network_paths_sfn, seoul_sobang, "Name") #여기서는 seoul_sn 사용
```
...
```{r}
library(TSP)
dist_matrix_network <- st_network_cost(seoul_sfn, from = seoul_sobang, to = seoul_sobang)
sobang_index <- st_nearest_feature(seoul_sobang, seoul_sfn) # 가장 가까이에 있는 도로 세그먼트 index

# 소방서간 최단 거리가 아니라 도로 세그먼트 간 최단 거리
row.names(dist_matrix_network) <- sobang_index
colnames(dist_matrix_network) <- sobang_index

# round(dist_matrix_network, 0) 

TSP_tour <- TSP::solve_TSP(TSP(units::drop_units(dist_matrix_network))) # TSP 문제를 해결
TSP_tour_index <- as.numeric(names(TSP_tour))
# TSP_tour_index # 도로 세그먼트 순서 index
...
```{r}
seoul_sn_cent <- st_centroid(seoul_sn) # 모든 도로 세그먼트의 센트로이드 생성

dist_cent_sobang <- st_network_cost(seoul_sfn, from = seoul_sn_cent, to = seoul_sobang) # 모든 도로 세그먼트의 센트로이드로부터 소방서까지의 네트워크 거리 계산

closest_cent <- seoul_sobang$Name[apply(dist_cent_sobang, 1, function(x) which(x == min(x))[1])] # 최단 거리에 있는 소방서 ID

seoul_sn$closest <- closest_cent
```
...
```{r}
dist_sample_sobang <- st_network_cost(seoul_sfn, from = network_sample, to = seoul_sobang)
...
```{r}
iso_jongro <- calc_isochrones(lines = seoul_sn, start_points = jongro_st, dists = c(2, 5, 10), weight = "LENGTH")
iso_jongro$fac_dist <- as.factor(iso_jongro$distance)
iso_jongro <- iso_jongro[order(-1*iso_jongro$distance),]
```
...
my_map <- tm_shape(seoul, is.master = TRUE) + tm_polygons(col = "gray98", border.col = "gray10", lwd = 1) +
  ...
  tm_shape(iso_jongro) + tm_lines(col = "fac_dist", lwd = 2, title.col = "Distance (km)", palette = c("1" = "#005f73", "5" = "#ca6702", "10" = "#9b2226"))+
  tm_shape(jongro_st) + tm_dots(col = "black", size = 1) + 
...
my_map <- tm_shape(seoul, is.master = TRUE) + tm_polygons(col = "gray98", border.col = "gray10", lwd = 1) +
  tm_shape(seoul_sn) + tm_lines(col = "gray") +
  tm_shape(iso_sp) + tm_polygons(col = "fac_dist", title = "Distance (km)", palette = c("1" = "#005f73", "5" = "#ca6702", "10" = "#9b2226"), alpha = 0.5, border.col = "white") +
  tm_shape(jongro_st) + tm_dots(col = "black", size = 1) + 
  tm_legend(legend.position = c(0.80, 0.05)) +
  tm_layout(inner.margins = c(0.06, 0.04, 0.04, 0.04), 
